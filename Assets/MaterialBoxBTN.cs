...
public class MaterialBoxBTN : MonoBehaviour
{
    ...
    public void OnClick(){
        ...
        for(int a = 3; a < Bag_slots.transform.childCount; a++){
            if(Bag_slots.transform.GetChild(a).GetChild(0).GetChild(0).GetComponent<ItemSlotImg>().getPicID() != 0){
                ...
                Bag_slots.transform.GetChild(a).GetChild(0).GetChild(0).GetComponent<ItemSlotImg>().ChangePic(ItemIndex);
    ...
    void Start()
    {
        transform.GetChild(0).GetComponent<ItemSlotImg>().ChangePic(ItemIndex);
    }
